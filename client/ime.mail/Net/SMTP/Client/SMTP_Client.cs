...
using ime.mail.Net.TCP;
using ime.mail.Net.AUTH;
using ime.mail.Net.DNS;
using ime.mail.Net.DNS.Client;
using ime.mail.Net.Mail;
using ime.mail.Net.MIME;
using ime.mail.Net.Mime;

namespace ime.mail.Net.SMTP.Client
{
    /// <summary>
            ...
            private void AuthReadResponseCompleted(ReadResponseAsyncOP op)
            {
                try{
                    ...
                    if(op.ReplyLines[0].ReplyCode == 334){
						...
						try
						{
							serverResponse = Convert.FromBase64String(op.ReplyLines[0].Text);
						}
						catch (Exception)
						{
							serverResponse = Encoding.UTF8.GetBytes(op.ReplyLines[0].Text);
						}
