...
namespace ime.mail.Net.MIME
{
    ...
    public class MIME_Encoding_EncodedWord
    {
		...
		{
			if (String.Compare(charset, "iso-8859-1", true) == 0)
            ...
            retVal = encodedword_regex.Replace(retVal,delegate(Match m){
                ...
                try{
                    if(string.Equals(m.Groups["encoding"].Value,"Q",StringComparison.InvariantCultureIgnoreCase)){
                        encodedWord =  MIME_Utils.QDecode(GetEncoding(m.Groups["charset"].Value),m.Groups["value"].Value);
                    }
                    else if(string.Equals(m.Groups["encoding"].Value,"B",StringComparison.InvariantCultureIgnoreCase)){
                        encodedWord = GetEncoding(m.Groups["charset"].Value).GetString(Net_Utils.FromBase64(Encoding.Default.GetBytes(m.Groups["value"].Value)));
                    }
